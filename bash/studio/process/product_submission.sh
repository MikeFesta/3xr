...
if [ $? -ne 0 ]; then exit $?; fi
xrs_log_info "Processing product submission: asset_uid ${asset_uid},  product_uid ${product_uid}, number ${submission_id}, folder ${folder}, asin ${asin}"
...
################################################################################
# 3. Render images (asynchronously, so status is no longer changed to 5/6)     #
################################################################################
...
data='{"assetUid":"'"${asset_uid}"'","name":"'"${name}"'","submissionId":'"${submission_id}"',"submissionNumber":'"${submission_number}"'}'
python3 ${PYTHON_DIR}/rabbit/queue_message.py render_master_images "${data}"
python3 ${PYTHON_DIR}/rabbit/queue_message.py render_qa_wireframes "${data}"
...
# TODO: this causes a bug when a previous upload had alpha in the diffuse and the artist wants to remove it
${BASH_3XR_DIR}/studio/textures/extract_opacity.sh -a ${asset_uid} -c ${submission_number}
${BASH_3XR_DIR}/studio/textures/combine_opacity_diffuse.sh -a ${asset_uid} -c ${submission_number}
