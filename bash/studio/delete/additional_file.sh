...
xrs_log_info "Deleting additional file for ${product_uid} filename: ${filename}"
file_to_delete="${PRODUCTS_DIR}/${product_uid}/reference/other/${filename}"
...
then
  ...
  then
    ...
    zip -d "${zip_file}" "reference/other/${filename}"
  fi
else
