    ...
    AssetSubmissionIssue.findByPk(req.body.id)
      .then(issue => {
        if (issue) {
          ...
          // @ts-ignore
          issue.resolutionTime = literal('CURRENT_TIMESTAMP');
          issue.resolved = true;
          return issue.save();
        } else {
          throw new Error(`Unable to resolve Issue. Cannot find a Issue with an id of ${req.body.id}.`);
        }
      })
      .then(saved_issue => {
      ...
      .catch((err: Error) => {
        Errors.resJson(res, err, 'Error Resolving Issue');
      });
  },
);
