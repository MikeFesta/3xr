...
xrs.triangulate(low_poly)

material_slots = low_poly.material_slots
if model_name+"_transparent" in bpy.data.objects:
  material_slots = [y for x in [low_poly.material_slots, bpy.data.objects[model_name+"_transparent"].material_slots] for y in x]
...
for material_slot in material_slots:
  ...
  diffuse_path = TEXTURE_DIR+material_name+"_2k_Diffuse.jpg"
  ...
  orm_path = TEXTURE_DIR+material_name+"_1k_ORM.png"
  ...
  normal_path = TEXTURE_DIR+material_name+"_1k_Normal.jpg"
  ...
  opacity_path = TEXTURE_DIR+material_name+"_2k_Opacity.jpg"
  if (os.path.isfile(opacity_path)):
      opacity_image = bpy.data.images.load(opacity_path, check_existing=True)
      opacity_node = nodes.new("ShaderNodeTexImage")
      opacity_node.location = (0, -100)
      opacity_node.width = 300
      opacity_node.image = opacity_image
      opacity_node.color_space = 'NONE'
      links.new(opacity_node.outputs[0], gltf_node.inputs[11])

  emissive_path = TEXTURE_DIR+material_name+"_1k_Emissive.jpg"
...
xrs.select_object(low_poly)
if model_name+"_transparent" in bpy.data.objects:
  #xrs.add_object_to_selection(bpy.data.objects[model_name+"_transparent"])
  bpy.data.objects[model_name+"_transparent"].select = True
bpy.ops.export_scene.glb(export_morph=False, export_layers=False, will_save_settings=True, export_current_frame=False, export_colors=False, export_selected=True, export_skins=False, export_animations=False, filepath=EXPORT_DIR+model_name+".glb")
