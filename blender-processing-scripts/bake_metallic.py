...
try:
  ...
  if (high_res == None):
    xrs.print_status("No high res mesh - extracting metallic value")
  elif (len(high_res.material_slots) == 1):
    xrs.print_status("Single Material Assigned to High Res - extracting metallic value")
    single_value = True
  else:
    # If there are multiple materials, check to see if they have different metallic values
    xrs.print_status("Multiple Materials Assigned to High Res - checking metallic values")
    value = xrs.get_material_metallic(high_res.material_slots[0].material)
    for slot in high_res.material_slots:
      other_value = xrs.get_material_metallic(slot.material)
      xrs.print_status(slot.material.name + " metallic = " + str(other_value))
      if (value != other_value):
        single_value = False

  if (single_value):
    # If there is only a single value, extract the values from low res
    for slot in low_res.material_slots:
      other_value = xrs.get_material_metallic(slot.material)
      value = xrs.get_material_metallic(slot.material)
      xrs.print_status("Metallic Value: " + str(value))
      with open(EXPORT_DIR + slot.material.name[0:-4] + "_Metallic.value", "w") as file:
        file.write("%i" % value)
  else:
    # If there are more than one materials, bake to a texture, high res to low res
    xrs.print_status("Baking multi-material metallic to texture")
    for slot in high_res.material_slots:
      # Set emit color to metallic value
      mat = slot.material
      metallic = xrs.get_material_metallic(mat) / 100
      emit = mat.node_tree.nodes.new(type="ShaderNodeEmission")
      emit.inputs[0].default_value = ((metallic, metallic, metallic, 1))
    ...
    for material_slot in low_res.material_slots:
      ...
      bake_mat = bpy.data.materials.new(name=material_name + "Metallic_Bake_Mat")
      ...
      bpy.ops.image.new(name=material_name + "_4k_Metallic", color=(1,1,1,1)) # This way lets you set a color
      img = bpy.data.images[material_name + "_4k_Metallic"]
      #img.use_alpha = False # Changed in 2.8
      img.generated_width = 4096
      img.generated_height = 4096
      img.filepath = EXPORT_DIR + material_name + "_4k_Metallic.png"
      img.file_format = "PNG"
    ...
    for material_slot in low_res.material_slots:
      name = material_slot.material.name[0:-17]
      img = bpy.data.images[name + "_4k_Metallic"]
      value_filepath = os.path.join(EXPORT_DIR, name + "_Metallic.value")
      if (os.path.exists(img.filepath) == False and os.path.exists(value_filepath) == False):
        xrs.print_status("Metallic does not exist, saving image")
